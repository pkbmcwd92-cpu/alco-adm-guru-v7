...
function runTests() {
  ...
  // -------------------------------------------------------------
  // TEST 6: Legacy Assessment Migration (Idempotent & Safe)
  // -------------------------------------------------------------
  ...
  const k13Plan: AssessmentPlan = {
    ...emptyPlan,
    title: 'Penilaian Harian KD 3.1',
    scopeType: 'TP',
    tpIds: ['kd-3.1'],
    instruments: [{ id: 'inst-k13', type: 'WRITTEN_TEST' }],
  };

  const valK13 = validateAssessmentPlan(k13Plan, k13Context);
  assert(valK13.valid, 'K13 KD reference validated successfully without fabricating Merdeka TP');
